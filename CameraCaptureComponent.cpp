...
#include "Controller/CameraCaptureComponent.h"
...
void UCameraCaptureComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	mDrawSceneToRenderTarget(DeltaTime);
}
...
void UCameraCaptureComponent::mDrawSceneToRenderTarget(float DeltaTime)
{
	...
	APlayerController* tempPlayerControler = Cast< APlayerController>(GetOwner());
	if (!::IsValid(tempPlayerControler))return;

	APawn* tempPlayer = tempPlayerControler->GetPawn();
	if (!::IsValid(tempPlayer))return;
	UCameraComponent* ActiveCameraComponent = Cast< UCameraComponent>(IMovementInterface::Execute_GetCameraComponent(tempPlayer));
	...
	if (TextureTarget && World)
	{
		...
		//修改
		{
			...
			//ViewFamily.EngineShowFlags.SetVisualizeHDR(true);
			...
			//ViewFamily.EngineShowFlags.SetScreenSpaceReflections(true);
			//ViewFamily.EngineShowFlags.EnableAdvancedFeatures();
			//ViewFamily.EngineShowFlags.SetSkyLighting
			//ViewFamily.bIsHDR = true;
			// 
			ViewFamily.EngineShowFlags.SetMotionBlur(0);
			...
			ViewFamily.EngineShowFlags.TemporalAA = false;
		...
		FSceneView* View = nullptr;
		{
			...
			//修改
			//ViewInitOptions.LODDistanceFactor = 100.0f;
			...
			View->OverridePostProcessSettings(PostProcessSettings, 1.0);
...
void UCameraCaptureComponent::mDrawSceneToRenderTarget_New()
{
	if (!::IsValid(TextureTarget))return;
}
